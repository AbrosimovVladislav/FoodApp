...
**FoodApp** — персональное мобильное приложение для планирования питания Влада.

Решает: планирование блюд на 6 дней недели, трекинг КБЖУ, управление холодильником, автогенерация списка покупок, голосовой ввод через OpenAI, аналитика питания.

Single-user, без авторизации. Stack: Next.js + Supabase + OpenAI gpt-4o-mini.

Полный план реализации: [`docs/plan.md`](docs/plan.md)
...
1. Прочитать [`docs/plan.md`](docs/plan.md)
2. Посмотреть на текущее состояние файлов (git status / структура папок)
3. Определить, на каком шаге мы находимся (Шаг 1–7)
4. Сообщить пользователю:
   - Текущий шаг и его цель
   - Что уже сделано в рамках этого шага
...
Приложение используется **только с телефона** (стоя на кухне).

- Все экраны проектируются и верстаются mobile-first
- Максимальная ширина контента: `max-w-md` (448px) — центрируется на больших экранах
- На десктопе выглядит как мобильное приложение (не растягивается на весь экран)
- Touch-friendly: кнопки минимум 44px, никаких hover-only интерактивных элементов
- Bottom navigation (не sidebar, не top nav)
- Приоритет: большие зоны касания, чёткая иерархия, минимум текста
